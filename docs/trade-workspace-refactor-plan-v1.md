...
**砚·交易台** — an A-share-focused analysis system running on the OpenClaw agent framework, serving a single user (Shanghai timezone) via Telegram. The system collects market data, generates daily/weekly analytical reports, and maintains long-term memory for prediction validation.
...
- **Telegram group**: configured in `config/claude_dispatch.json`
